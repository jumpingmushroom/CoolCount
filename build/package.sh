...
PROJ="$ROOT/src/CoolCount/CoolCount.csproj"
DLL="$ROOT/src/CoolCount/bin/Release/net472/CoolCount.dll"
...
ASM_VERSION=$(grep -oP '(?<=PluginVersion = ")[^"]+' "$ROOT/src/CoolCount/Plugin.cs")
...
echo "==> staging"
rm -rf "$STAGE"
mkdir -p "$STAGE/plugins/CoolCount"
cp "$DLL"                        "$STAGE/plugins/CoolCount/"
...
OUT="$ROOT/dist/CoolCount-$VERSION.zip"
rm -f "$OUT"
( cd "$STAGE" && zip -qr "$OUT" . )
