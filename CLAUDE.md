...
- Commit only when asked. Version bumps touch three places together: `PluginVersion` in
  `src/CoolCount/Plugin.cs`, `<Version>` in the csproj, and `version_number` in
...
- Reference assemblies live in `lib/` (gitignored). Pull them from the rig's
  `valheim_Data/Managed`, not from an older sibling mod. No Jotunn: the mod depends on BepInEx
